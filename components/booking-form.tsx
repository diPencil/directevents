...
export function BookingForm() {
  ...
  const bookingApiBaseUrl = "https://directevents.vercel.app"
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Send to backend API
      const response = await fetch(`${bookingApiBaseUrl}/api/booking`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.message || (language === "ar" ? "حدث خطأ في الإرسال" : "Failed to submit booking"))
      }
      ...
      setTimeout(() => {
        window.open(`https://wa.me/201130720860?text=${whatsappMessage}`, "_blank")
      }, 2000)
                ...
                <div className="bg-slate-100 rounded-2xl p-1.5 flex mb-6">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${form.watch("bookingType") === opt.value
                                ...
                                <FormItem dir={isRtl ? "rtl" : "ltr"}>
                                  ...
                                  <FormControl>
                                    <Input
                                      type="date"
                                      {...field}
                                      className={`bg-slate-50 border-slate-200 h-11 rounded-xl w-full ${isRtl
